...
<html> \n \
 ...
 \n \
      function onAddRemoteStream(event) { \n \
        ...
        if (!html5VideoElement.srcObject)\n\
...
static GstWebRTCPriorityType _priority_from_string(const gchar* s)
{
    ...
    g_return_val_if_fail(klass, (GstWebRTCPriorityType)0);
    ...
    return (GstWebRTCPriorityType)0;
}

ReceiverEntry* create_receiver_entry(SoupWebsocketConnection* connection)
{
    GError* error = nullptr;
    ReceiverEntry* receiver_entry = nullptr;
    ...
    SoupWebsocketDataType data_type, GBytes* message, gpointer user_data)
{
    ...
    if (g_strcmp0(type_string, "sdp") == 0) {
        ...
        mline_index =
            (guint)json_object_get_int_member(data_json_object, "sdpMLineIndex");
